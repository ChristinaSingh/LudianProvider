...
*/
        ...
        fun formatDates(dateString: String): List<String> {
            ...
            // Split the input string by commas without spaces
            val dateList = dateString.split(",") // Use split(",") to handle input like '2024-12-11,2024-12-12'

            // Format each date and store them in a list
            return dateList.map { dateString ->
                // Parse each date string into LocalDate and format it
                val date = LocalDate.parse(dateString.trim()) // Trim any unwanted spaces around the date
                date.format(outputFormatter)
            }
        }
